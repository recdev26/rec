...
export default function BlogCard({
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden bg-[var(--color-off-white)] shadow-[0_18px_44px_rgba(11,46,44,0.08)]">
      <div className={`relative h-80 overflow-hidden ${toneClasses[tone]}`}>
        ...
        <div className="absolute left-7 top-7 flex h-20 w-20 flex-col items-center justify-center bg-[var(--color-accent)] text-white">
          <span className="font-heading text-3xl leading-none font-bold">{dateDay}</span>
      ...
      <div className="flex flex-1 flex-col p-8">
        <h3 className="min-h-24 font-heading text-3xl leading-tight font-bold text-[var(--color-text)]">
          {title}
        </h3>
        <p className="mt-6 flex-1 text-lg leading-8 text-[var(--color-gray-dark)]">
          {excerpt}
        </p>
