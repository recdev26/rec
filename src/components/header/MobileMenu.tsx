...
interface MobileMenuProps {
  isMenuOpen: boolean
  locale: Locale
  openMobileSectionKey: string | null
  pathname: string
  onClose: () => void
...
export default function MobileMenu({
  ...
  const expandLabelPrefix = locale === 'en' ? 'Expand' : 'Expandir'
        ...
        <nav className="flex-1 overflow-y-auto">
          {navigationItems.map((item) => {
            ...
            if (isDropdownItem(item)) {
              const isOpen = openMobileSectionKey === item.key
                ...
                <div key={item.label} className="border-b border-[var(--color-gray-light)]">
                  <div className="flex items-center justify-between px-7 py-6">
                    ...
                    <button
                      ...
                      aria-label={`${expandLabelPrefix} ${item.label}`}
